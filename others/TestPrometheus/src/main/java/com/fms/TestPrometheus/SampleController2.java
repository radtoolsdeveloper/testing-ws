...
public class SampleController2 {
	...
	public String carrierJobMetrics() {
		...
		// Gauge gauge = Gauge.builder("carrier.jobs.with.status.size", list,
		// List::size).register(simpleMeterRegistry);
	...
	@ResponseBody
	public String carrierJobPMetrics(PrometheusMeterRegistry prometheusMeterRegistry) {
		okCounter = Counter.builder("carrier.jobs.with.status.ok").tags("version", "v1").description("Carrier Jobs with status OK").register(prometheusMeterRegistry);
